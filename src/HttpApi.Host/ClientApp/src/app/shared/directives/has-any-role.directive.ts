...
export class HasAnyRoleDirective implements OnInit, OnDestroy {
  ...
  @Input() set appHasAnyRole(roles: string | string[]) {
  ...
  private updateView(): void {
    this.authService.getUser()
      .pipe(takeUntil(this.destroy$))
      .subscribe(user => {
        const hasAnyRole = this.checkAnyRole(user);
        if (hasAnyRole) {
          this.showView();
        } else {
          this.hideView();
  ...
  private checkAnyRole(user: any): boolean {
    if (!user) {
      return false;
    }

    if (this.requiredRoles.length === 0) {
      // If no specific roles required, just check if user has any roles
